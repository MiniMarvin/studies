...
private:
    ...
    node* simple_rotation(node *el, int side) {
        ...
        if(side == -1) { // Left subtree
            ...
            el->set_left(el->get_left()->get_right()); // Make the node points to him's son's son.
            clone->set_right(el); // Make him's son points to him
        }
        
        else if(side == 1) { // Right subtree
            clone = el->get_right(); // Saves the pointer
            el->set_right(el->get_right()->get_left()); // Make the node points to him's son's son.
            clone->set_right(el); // Make him's son points to him
        }
    ...
    node* add(int value, node *el) { 
        ...
        if(el->get_value() < value) {
            ...
            if(child_balance != 0) el->grow_balance(); // Verify if the height of the tree has grown
            ...
            child_balance = ret->get_balance();
            if(child_balance != 0) el->drop_balance(); // Verify if the height of the tree has grown
